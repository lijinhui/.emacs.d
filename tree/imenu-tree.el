...
(defun imenu-tree-expand-add-marker-for-python-mode (python-imenu-list buf)
  
  (when (listp python-imenu-list)
    (dolist (item python-imenu-list)
      (if (listp (cdr item))
	  (imenu-tree-expand-add-marker-for-python-mode (cdr item) buf)
	(if (and (consp item) (integerp (cdr item)))
	    (setcdr item (move-marker (make-marker) (cdr item) buf))
	    ))))
  )
...
(defun imenu-tree-expand (tree)
  (or (widget-get tree :args)
      (let* ((buf (widget-get tree :buffer))
             ...
             (idx (save-excursion
                    (set-buffer buf)
		    (setq python-mode-imenu-function (string= imenu-create-index-function "python-imenu-create-index"))
                    (reverse (imenu--make-index-alist t))))
             widgets)
	(if python-mode-imenu-function (imenu-tree-expand-add-marker-for-python-mode idx buf))
        (dolist (item idx)
          (if (listp (cdr item))
              (push `(tree-widget
                      :node (push-button
            ...
            (unless (and (string= "*Rescan*" (car item))
              ...
              (push `(push-button
                      :tag ,(car item)
                      ;;:imenu-marker ,(if python-mode-imenu-function (move-marker (make-marker) (cdr item) buf) (cdr item))
		      :imenu-marker ,(cdr item)
